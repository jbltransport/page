...
class TMSStore {
    ...
    recordDriverAdvance(driverId, advData) {
        ...
        // If linked to a trip, record payment towards trip supplier/driver payable (deducts from Trans Due)
        if (advData.tripId && amount > 0) {
            const trips = this.getTrips();
            const trip = trips.find(t => t.id === advData.tripId);
            if (trip) {
                // Interlink: assign driver to this trip if unassigned
                if (!trip.driverId || trip.driverName === 'Unassigned') {
                ...
                if (advData.type === 'gave') {
                    // Payment to driver for this trip: deducts from trip trans due / supplierBalance
                    const supplierDisplayName = (trip.supplierName && trip.supplierName !== 'Self Owned') 
                        ? `${trip.supplierName} (Driver: ${drivers[idx].name})` 
                        : `Driver: ${drivers[idx].name}`;
                    ...
                    this.addPayment({
                        ...
                        referenceNo: 'DRV-ADV-' + Math.floor(100000 + Math.random() * 900000),
                        notes: advData.notes ? `Driver Advance (${drivers[idx].name}): ${advData.notes}` : `Driver Advance to ${drivers[idx].name} for trip ${trip.lrNo}`
                    });
                } else if (advData.type === 'got') {
                    // Driver refunded excess money: reduces advance paid towards trip
                    ...
                    this.updateTrip(trip.id, {
                        ...
                        truckHireCost: trip.supplierBilling?.truckHireCost || 0
                    });
                }
            }
        }
