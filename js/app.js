...
function openNewTripModal(tripId = null) {
    ...
    if (tripId) {
        ...
        if (trip) {
            ...
            // Sync Search Inputs with Selected Trip Values
            const party = window.tmsStore.getParties().find(p => p.id === trip.partyId);
            const partyInp = document.getElementById('tripPartySearchInput');
            if (partyInp) partyInp.value = party ? party.name : (trip.partyName || '');
            document.getElementById('clearTripPartySearchBtn')?.classList.toggle('hidden', !partyInp?.value);

            const truck = window.tmsStore.getTrucks().find(t => t.id === trip.truckId);
            const truckInp = document.getElementById('tripTruckSearchInput');
            if (truckInp) truckInp.value = truck ? truck.truckNo : (trip.truckNo || '');
            document.getElementById('clearTripTruckSearchBtn')?.classList.toggle('hidden', !truckInp?.value);

            const supplier = window.tmsStore.getSuppliers().find(s => s.id === trip.supplierId);
            const suppInp = document.getElementById('tripSupplierSearchInput');
            if (suppInp) suppInp.value = supplier ? supplier.name : (trip.supplierId ? (trip.supplierName || '') : 'Self Owned / Direct Fleet');
            document.getElementById('clearTripSupplierSearchBtn')?.classList.toggle('hidden', !suppInp?.value);

            const driver = window.tmsStore.getDrivers().find(d => d.id === trip.driverId);
            const drvInp = document.getElementById('tripDriverSearchInput');
            if (drvInp) drvInp.value = driver ? driver.name : (trip.driverName || '');
            document.getElementById('clearTripDriverSearchBtn')?.classList.toggle('hidden', !drvInp?.value);
        }
    } else {
        ...
        // Clear Search Inputs & Suggestions
        ['tripPartySearchInput', 'tripTruckSearchInput', 'tripSupplierSearchInput', 'tripDriverSearchInput'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
        ['clearTripPartySearchBtn', 'clearTripTruckSearchBtn', 'clearTripSupplierSearchBtn', 'clearTripDriverSearchBtn'].forEach(id => {
            document.getElementById(id)?.classList.add('hidden');
        });
        ['tripPartySuggestions', 'tripTruckSuggestions', 'tripSupplierSuggestions', 'tripDriverSuggestions'].forEach(id => {
            document.getElementById(id)?.classList.add('hidden');
        });
    }
...
// -------------------------------------------------------------
// Live Searchable Dropdowns for Parties, Trucks, Suppliers, Drivers
// -------------------------------------------------------------
...
// 1. PARTY DROPDOWN WITH LIVE SEARCH
function populatePartyDropdown(selectedId = null, filterQuery = '') {
    ...
    let defaultText = '-- Select Consignor Party --';
    if (q) {
        defaultText = filtered.length > 0 ? `-- ${filtered.length} Matching Parties --` : '-- No matching party found --';
    }

    let html = `<option value="">${defaultText}</option>`;
    html += filtered.map(p => {
        const isSel = selectedId ? (selectedId === p.id) : (q && filtered.length === 1);
        return `<option value="${p.id}" ${isSel ? 'selected' : ''}>${p.name} (${p.city || 'N/A'})${p.phone ? ' · ' + p.phone : ''}</option>`;
    }).join('');

    // Preserve selected party if not in current filter
    if (selectedId && !filtered.some(p => p.id === selectedId)) {
        const selP = parties.find(p => p.id === selectedId);
        if (selP) {
            html += `<option value="${selP.id}" selected>${selP.name} (${selP.city || 'N/A'}) [Selected]</option>`;
        }
    }
    ...
    renderPartySuggestions(filtered, q);

    const clearBtn = document.getElementById('clearTripPartySearchBtn');
    if (clearBtn) clearBtn.classList.toggle('hidden', !q);
}

function filterTripPartyOptions(query) {
    const currentVal = document.getElementById('tripPartySelect')?.value;
    populatePartyDropdown(currentVal, query);
}

function renderPartySuggestions(parties, query) {
    const box = document.getElementById('tripPartySuggestions');
    if (!box) return;
    if (!query) {
    ...
    if (parties.length === 0) {
        box.innerHTML = `<div class="px-3.5 py-3 text-center text-slate-400 text-xs">No matching party found</div>`;
        return;
    }

    box.innerHTML = parties.slice(0, 8).map(p => `
        <div class="px-3.5 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors group" onclick="selectTripParty('${p.id}')">
            <div>
                <div class="font-bold text-slate-800 group-hover:text-blue-700 text-xs">${escapeSearchHtml(p.name)}</div>
                <div class="text-[11px] text-slate-500">${escapeSearchHtml(p.city || 'N/A')} ${p.phone ? '• ' + escapeSearchHtml(p.phone) : ''}</div>
            </div>
            <span class="text-[10px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-colors">Select</span>
        </div>
    `).join('');
}

function selectTripParty(partyId) {
    const party = window.tmsStore.getParties().find(p => p.id === partyId);
    if (!party) return;

    const input = document.getElementById('tripPartySearchInput');
    if (input) input.value = party.name;

    populatePartyDropdown(partyId, '');
    const select = document.getElementById('tripPartySelect');
    if (select) {
    ...
    document.getElementById('tripPartySuggestions')?.classList.add('hidden');
    document.getElementById('clearTripPartySearchBtn')?.classList.remove('hidden');
}

function clearTripPartySearch() {
    const input = document.getElementById('tripPartySearchInput');
    if (input) input.value = '';
    const currentVal = document.getElementById('tripPartySelect')?.value;
    populatePartyDropdown(currentVal, '');
    document.getElementById('tripPartySuggestions')?.classList.add('hidden');
    document.getElementById('clearTripPartySearchBtn')?.classList.add('hidden');
}

// 2. TRUCK DROPDOWN WITH LIVE SEARCH
function populateTruckDropdown(selectedId = null, filterQuery = '') {
    ...
    if (q) {
        filtered = trucks.filter(t => 
            ...
            (t.truckType && t.truckType.toLowerCase().includes(q)) ||
            (t.ownership && t.ownership.toLowerCase().includes(q))
        );
        filtered.sort((a, b) => {
            const aStarts = (a.truckNo || '').toLowerCase().startsWith(q);
            const bStarts = (b.truckNo || '').toLowerCase().startsWith(q);
            if (aStarts && !bStarts) return -1;
            if (!aStarts && bStarts) return 1;
    ...
    let defaultText = '-- Select Truck / Vehicle --';
    if (q) {
        defaultText = filtered.length > 0 ? `-- ${filtered.length} Matching Trucks --` : '-- No matching truck found --';
    }

    let html = `<option value="">${defaultText}</option>`;
    html += filtered.map(t => {
        const isSel = selectedId ? (selectedId === t.id) : (q && filtered.length === 1);
        return `<option value="${t.id}" data-ownership="${t.ownership}" data-supplier="${t.supplierId}" ${isSel ? 'selected' : ''}>${t.truckNo} [${t.ownership === 'My' ? 'OWN FLEET' : 'MARKET'}] - ${t.truckType}</option>`;
    }).join('');

    if (selectedId && !filtered.some(t => t.id === selectedId)) {
        const selT = trucks.find(t => t.id === selectedId);
        if (selT) {
            html += `<option value="${selT.id}" data-ownership="${selT.ownership}" data-supplier="${selT.supplierId}" selected>${selT.truckNo} [${selT.ownership === 'My' ? 'OWN FLEET' : 'MARKET'}] [Selected]</option>`;
        }
    }
    ...
    renderTruckSuggestions(filtered, q);

    const clearBtn = document.getElementById('clearTripTruckSearchBtn');
    if (clearBtn) clearBtn.classList.toggle('hidden', !q);
}

function filterTripTruckOptions(query) {
    const currentVal = document.getElementById('tripTruckSelect')?.value;
    populateTruckDropdown(currentVal, query);
}

function renderTruckSuggestions(trucks, query) {
    const box = document.getElementById('tripTruckSuggestions');
    if (!box) return;
    if (!query) {
        box.classList.add('hidden');
        box.innerHTML = '';
    ...
    box.classList.remove('hidden');
    if (trucks.length === 0) {
        box.innerHTML = `<div class="px-3.5 py-3 text-center text-slate-400 text-xs">No matching truck found</div>`;
        return;
    }

    box.innerHTML = trucks.slice(0, 8).map(t => `
        <div class="px-3.5 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors group" onclick="selectTripTruck('${t.id}')">
            <div>
                <div class="font-bold text-slate-800 group-hover:text-blue-700 text-xs">${escapeSearchHtml(t.truckNo)}</div>
                <div class="text-[11px] text-slate-500">${escapeSearchHtml(t.truckType || '')}</div>
            </div>
            <span class="text-[10px] font-semibold ${t.ownership === 'My' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : 'text-blue-600 bg-blue-50 border-blue-100'} px-2 py-0.5 rounded border">
                ${t.ownership === 'My' ? 'OWN FLEET' : 'MARKET'}
            </span>
        </div>
    `).join('');
}

function selectTripTruck(truckId) {
    const truck = window.tmsStore.getTrucks().find(t => t.id === truckId);
    if (!truck) return;

    const input = document.getElementById('tripTruckSearchInput');
    if (input) input.value = truck.truckNo;

    populateTruckDropdown(truckId, '');
    const select = document.getElementById('tripTruckSelect');
    if (select) {
    ...
    document.getElementById('tripTruckSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripTruckSearchBtn')?.classList.remove('hidden');
}

function clearTripTruckSearch() {
    const input = document.getElementById('tripTruckSearchInput');
    if (input) input.value = '';
    const currentVal = document.getElementById('tripTruckSelect')?.value;
    populateTruckDropdown(currentVal, '');
    document.getElementById('tripTruckSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripTruckSearchBtn')?.classList.add('hidden');
}

// 3. SUPPLIER DROPDOWN WITH LIVE SEARCH
function populateSupplierDropdown(selectedId = null, filterQuery = '') {
    ...
    let defaultText = 'Self Owned / Direct Fleet';
    if (q && !'self owned / direct fleet'.includes(q)) {
        defaultText = filtered.length > 0 ? `-- ${filtered.length} Matching Suppliers --` : '-- No matching supplier found --';
    }

    let html = `<option value="">${defaultText}</option>`;
    html += filtered.map(s => {
        const isSel = selectedId ? (selectedId === s.id) : (q && filtered.length === 1);
        return `<option value="${s.id}" ${isSel ? 'selected' : ''}>${s.name} (${s.city || 'Transporter'})${s.phone ? ' · ' + s.phone : ''}</option>`;
    }).join('');

    if (selectedId && !filtered.some(s => s.id === selectedId)) {
        const selS = suppliers.find(s => s.id === selectedId);
        if (selS) {
            html += `<option value="${selS.id}" selected>${selS.name} (${selS.city || 'Transporter'}) [Selected]</option>`;
        }
    }
    ...
    renderSupplierSuggestions(filtered, q);

    const clearBtn = document.getElementById('clearTripSupplierSearchBtn');
    if (clearBtn) clearBtn.classList.toggle('hidden', !q);
}

function filterTripSupplierOptions(query) {
    const currentVal = document.getElementById('tripSupplierSelect')?.value;
    populateSupplierDropdown(currentVal, query);
}

function renderSupplierSuggestions(suppliers, query) {
    const box = document.getElementById('tripSupplierSuggestions');
    if (!box) return;
    if (!query) {
        box.classList.add('hidden');
        box.innerHTML = '';
    ...
    box.classList.remove('hidden');
    let itemsHtml = '';
    if ('self owned / direct fleet'.includes(query)) {
        itemsHtml += `
            <div class="px-3.5 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors group" onclick="selectTripSupplier('')">
                <div>
                    <div class="font-bold text-slate-800 group-hover:text-blue-700 text-xs">Self Owned / Direct Fleet</div>
                    <div class="text-[11px] text-slate-500">No external transporter hire</div>
                </div>
                <span class="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">Default</span>
            </div>
        `;
    }

    if (suppliers.length === 0 && !itemsHtml) {
        box.innerHTML = `<div class="px-3.5 py-3 text-center text-slate-400 text-xs">No matching supplier found</div>`;
        return;
    }

    itemsHtml += suppliers.slice(0, 8).map(s => `
        <div class="px-3.5 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors group" onclick="selectTripSupplier('${s.id}')">
            <div>
                <div class="font-bold text-slate-800 group-hover:text-blue-700 text-xs">${escapeSearchHtml(s.name)}</div>
                <div class="text-[11px] text-slate-500">${escapeSearchHtml(s.city || 'Transporter')} ${s.phone ? '• ' + escapeSearchHtml(s.phone) : ''}</div>
            </div>
            <span class="text-[10px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-colors">Select</span>
        </div>
    `).join('');
...
function selectTripSupplier(supplierId) {
    const input = document.getElementById('tripSupplierSearchInput');
    if (!supplierId) {
        if (input) input.value = 'Self Owned / Direct Fleet';
        populateSupplierDropdown('', '');
        const select = document.getElementById('tripSupplierSelect');
        if (select) select.value = '';
    } else {
        const supplier = window.tmsStore.getSuppliers().find(s => s.id === supplierId);
        if (supplier && input) input.value = supplier.name;
    ...
    document.getElementById('tripSupplierSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripSupplierSearchBtn')?.classList.remove('hidden');
}

function clearTripSupplierSearch() {
    const input = document.getElementById('tripSupplierSearchInput');
    if (input) input.value = '';
    const currentVal = document.getElementById('tripSupplierSelect')?.value;
    populateSupplierDropdown(currentVal, '');
    document.getElementById('tripSupplierSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripSupplierSearchBtn')?.classList.add('hidden');
}

// 4. DRIVER DROPDOWN WITH LIVE SEARCH
function populateDriverDropdown(selectedId = null, filterQuery = '') {
    ...
    let defaultText = '-- Select Driver --';
    if (q) {
        defaultText = filtered.length > 0 ? `-- ${filtered.length} Matching Drivers --` : '-- No matching driver found --';
    }

    let html = `<option value="">${defaultText}</option>`;
    html += filtered.map(d => {
        const isSel = selectedId ? (selectedId === d.id) : (q && filtered.length === 1);
        return `<option value="${d.id}" ${isSel ? 'selected' : ''}>${d.name} (${d.mobile || 'No Mobile'}) - ${d.status || 'Available'}</option>`;
    }).join('');

    if (selectedId && !filtered.some(d => d.id === selectedId)) {
        const selD = drivers.find(d => d.id === selectedId);
        if (selD) {
            html += `<option value="${selD.id}" selected>${selD.name} (${selD.mobile || ''}) [Selected]</option>`;
        }
    }
    ...
    renderDriverSuggestions(filtered, q);

    const clearBtn = document.getElementById('clearTripDriverSearchBtn');
    if (clearBtn) clearBtn.classList.toggle('hidden', !q);
}

function filterTripDriverOptions(query) {
    const currentVal = document.getElementById('tripDriverSelect')?.value;
    populateDriverDropdown(currentVal, query);
}

function renderDriverSuggestions(drivers, query) {
    const box = document.getElementById('tripDriverSuggestions');
    if (!box) return;
    if (!query) {
        box.classList.add('hidden');
        box.innerHTML = '';
    ...
    box.classList.remove('hidden');
    if (drivers.length === 0) {
        box.innerHTML = `<div class="px-3.5 py-3 text-center text-slate-400 text-xs">No matching driver found</div>`;
        return;
    }

    box.innerHTML = drivers.slice(0, 8).map(d => `
        <div class="px-3.5 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors group" onclick="selectTripDriver('${d.id}')">
            <div>
                <div class="font-bold text-slate-800 group-hover:text-blue-700 text-xs">${escapeSearchHtml(d.name)}</div>
                <div class="text-[11px] text-slate-500">${escapeSearchHtml(d.mobile || 'No Phone')} ${d.licenseNo ? '• DL: ' + escapeSearchHtml(d.licenseNo) : ''}</div>
            </div>
            <span class="text-[10px] font-semibold ${d.status === 'On Trip' ? 'text-amber-700 bg-amber-50 border-amber-200' : 'text-emerald-700 bg-emerald-50 border-emerald-200'} px-2 py-0.5 rounded border">
                ${escapeSearchHtml(d.status || 'Available')}
            </span>
        </div>
    `).join('');
}

function selectTripDriver(driverId) {
    const driver = window.tmsStore.getDrivers().find(d => d.id === driverId);
    if (!driver) return;

    const input = document.getElementById('tripDriverSearchInput');
    if (input) input.value = driver.name;

    populateDriverDropdown(driverId, '');
    const select = document.getElementById('tripDriverSelect');
    if (select) {
    ...
    document.getElementById('tripDriverSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripDriverSearchBtn')?.classList.remove('hidden');
}

function clearTripDriverSearch() {
    const input = document.getElementById('tripDriverSearchInput');
    if (input) input.value = '';
    const currentVal = document.getElementById('tripDriverSelect')?.value;
    populateDriverDropdown(currentVal, '');
    document.getElementById('tripDriverSuggestions')?.classList.add('hidden');
    document.getElementById('clearTripDriverSearchBtn')?.classList.add('hidden');
}
...
function setupTripCalculationListeners() {
    ...
    truckSelect?.addEventListener('change', () => {
        ...
        if (ownership === 'My') {
            ...
            const suppInp = document.getElementById('tripSupplierSearchInput');
            if (suppInp) suppInp.value = 'Self Owned / Direct Fleet';
            document.getElementById('clearTripSupplierSearchBtn')?.classList.remove('hidden');
        } else if (supplierId) {
            document.getElementById('tripSupplierSelect').value = supplierId;
            const sup = window.tmsStore.getSuppliers().find(s => s.id === supplierId);
            const suppInp = document.getElementById('tripSupplierSearchInput');
            if (suppInp && sup) {
                suppInp.value = sup.name;
                document.getElementById('clearTripSupplierSearchBtn')?.classList.remove('hidden');
            }
        }
    });
...
function handleSaveQuickParty(e) {
    ...
    const partyInp = document.getElementById('tripPartySearchInput');
    if (partyInp) partyInp.value = newParty.name;
    document.getElementById('clearTripPartySearchBtn')?.classList.remove('hidden');
...
function handleSaveQuickTruck(e) {
    ...
    const truckInp = document.getElementById('tripTruckSearchInput');
    if (truckInp) truckInp.value = newTruck.truckNo;
    document.getElementById('clearTripTruckSearchBtn')?.classList.remove('hidden');
...
function handleSaveQuickSupplier(e) {
    ...
    if (quickSupplierContext === 'truck') {
        ...
        const suppInp = document.getElementById('tripSupplierSearchInput');
        if (suppInp) suppInp.value = newSupplier.name;
        document.getElementById('clearTripSupplierSearchBtn')?.classList.remove('hidden');
    }
...
function handleSaveQuickDriver(e) {
    ...
    const drvInp = document.getElementById('tripDriverSearchInput');
    if (drvInp) drvInp.value = newDriver.name;
    document.getElementById('clearTripDriverSearchBtn')?.classList.remove('hidden');
...
function setupSearchListeners() {
    ...
    // Sync Trip Form Selects back to search inputs when changed directly
    document.getElementById('tripPartySelect')?.addEventListener('change', (e) => {
        const pId = e.target.value;
        const p = window.tmsStore.getParties().find(x => x.id === pId);
        const input = document.getElementById('tripPartySearchInput');
        if (input) input.value = p ? p.name : '';
        document.getElementById('clearTripPartySearchBtn')?.classList.toggle('hidden', !p);
        document.getElementById('tripPartySuggestions')?.classList.add('hidden');
    });

    document.getElementById('tripTruckSelect')?.addEventListener('change', (e) => {
        const tId = e.target.value;
        const t = window.tmsStore.getTrucks().find(x => x.id === tId);
        const input = document.getElementById('tripTruckSearchInput');
        if (input) input.value = t ? t.truckNo : '';
        document.getElementById('clearTripTruckSearchBtn')?.classList.toggle('hidden', !t);
        document.getElementById('tripTruckSuggestions')?.classList.add('hidden');
    });

    document.getElementById('tripSupplierSelect')?.addEventListener('change', (e) => {
        const sId = e.target.value;
        const s = window.tmsStore.getSuppliers().find(x => x.id === sId);
        const input = document.getElementById('tripSupplierSearchInput');
        if (input) input.value = s ? s.name : (sId ? '' : 'Self Owned / Direct Fleet');
        document.getElementById('clearTripSupplierSearchBtn')?.classList.toggle('hidden', !s && !(!sId));
        document.getElementById('tripSupplierSuggestions')?.classList.add('hidden');
    });

    document.getElementById('tripDriverSelect')?.addEventListener('change', (e) => {
        const dId = e.target.value;
        const d = window.tmsStore.getDrivers().find(x => x.id === dId);
        const input = document.getElementById('tripDriverSearchInput');
        if (input) input.value = d ? d.name : '';
        document.getElementById('clearTripDriverSearchBtn')?.classList.toggle('hidden', !d);
        document.getElementById('tripDriverSuggestions')?.classList.add('hidden');
    });

    // Keyboard handlers for search inputs (Enter selects top suggestion or closes, Escape closes)
    const tripSearchConfigs = [
        { inputId: 'tripPartySearchInput', boxId: 'tripPartySuggestions' },
        { inputId: 'tripTruckSearchInput', boxId: 'tripTruckSuggestions' },
        { inputId: 'tripSupplierSearchInput', boxId: 'tripSupplierSuggestions' },
        { inputId: 'tripDriverSearchInput', boxId: 'tripDriverSuggestions' }
    ];

    tripSearchConfigs.forEach(({ inputId, boxId }) => {
        ...
        inp.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                const firstItem = box?.querySelector('.group');
                if (firstItem && !box.classList.contains('hidden')) {
                    firstItem.click();
                } else {
                    box?.classList.add('hidden');
                }
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('#tripPartySearchWrapper')) {
            document.getElementById('tripPartySuggestions')?.classList.add('hidden');
        }
        if (!e.target.closest('#tripTruckSearchWrapper')) {
            document.getElementById('tripTruckSuggestions')?.classList.add('hidden');
        }
        if (!e.target.closest('#tripSupplierSearchWrapper')) {
            document.getElementById('tripSupplierSuggestions')?.classList.add('hidden');
        }
        if (!e.target.closest('#tripDriverSearchWrapper')) {
            document.getElementById('tripDriverSuggestions')?.classList.add('hidden');
        }
    });
}
