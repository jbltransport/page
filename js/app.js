...
function openDriverAdvanceModal(driverId) {
    ...
    const trips = window.tmsStore.getTrips().filter(t => t.driverId === driverId || t.status === 'In Transit');
    ...
    if (select) {
        select.innerHTML = '<option value="">-- General Khata Advance (No Trip) --</option>' + 
            trips.map(t => `<option value="${t.id}">${t.lrNo} - ${t.route.origin} &rarr; ${t.route.destination} (${t.status})</option>`).join('');
    }
...
function handleSaveDriverAdvance(e) {
    ...
    showToast(`Driver transaction of ${formatINR(amount)} recorded for ${driver?.name || 'Driver'}!`);
