...
function handleSaveQuickSupplier(e) {
    ...
    showToast(`Supplier "${newSupplier.name}" added and selected!`);
}
...
function renderSuppliers() {
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center py-8 text-slate-400">No suppliers/transporters found.</td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = filtered.map(s => {
        ...
        return `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="text-center">
                    <span class="font-bold text-slate-800">${supTrips.length}</span>
                    <span class="block text-xs text-slate-400">Hire: ${formatINR(totalHire)}</span>
                </td>
                <td>
                    <div class="font-bold ${balanceDue > 0 ? 'text-rose-600' : 'text-emerald-600'}">
                        ${formatINR(balanceDue)}
                    </div>
                </td>
                <td class="text-right whitespace-nowrap">
                    <button onclick="openSettleSupplierModal('${s.id}')" class="px-2.5 py-1 text-xs font-semibold bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded">
                        Settle Balance
                    </button>
                    <button onclick="deleteSupplierConfirm('${s.id}')" class="p-1 text-slate-400 hover:text-rose-600 ml-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    </button>
                </td>
            </tr>
        ...
        return `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td>
                    ...
                    <div class="text-xs text-slate-500 font-mono">${d.upiId || ''}</div>
                </td>
                <td>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${d.status === 'On Trip' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}">
                        ${d.status}
                    </span>
                </td>
                ...
                <td class="text-right whitespace-nowrap">
                    <button onclick="openDriverAdvanceModal('${d.id}')" class="px-2 py-1 text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 rounded">
                        Give Advance
                    </button>
                    <button onclick="deleteDriverConfirm('${d.id}')" class="p-1 text-slate-400 hover:text-rose-600 ml-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    </button>
                </td>
            </tr>
